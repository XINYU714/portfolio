...
const website = computed(() => projectItem.value?.website)
...
const titleElementList = computed(() =>
  articleRef.value
    ? Array.from(articleRef.value.querySelectorAll<HTMLElement>('.article-title'))
    : []
)
const handleScrollSection = (index: number) => titleElementList.value[index]?.scrollIntoView({ behavior: 'smooth' })
useIntersectionObserver(
  titleElementList,
  (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        const titleIndex = titleElementList.value.indexOf(entry.target as HTMLElement)
        if (titleIndex !== -1) {
    ...
    <div class="main-container py-10">
        <section class="py-10 lg:py-20 flex justify-between items-center gap-10 flex-col lg:flex-row">
          <div class="flex-1 lg:flex-6">
            ...
            <div class="mt-5">
              <a v-if="website?.official" :href="website.official" target="_blank" class="block w-fit ml-auto hover:text-accent transition-colors">
              ...
              <a v-if="website?.github" :href="website.github" target="_blank" class="block w-fit ml-auto hover:text-accent transition-colors">
        ...
        <section class="flex gap-10 justify-between">
          <div ref="articleRef" class="flex-8">
          ...
          <div v-if="tocList.length > 0" class="sticky top-20 flex-3 hidden lg:block self-start justify-items-center">
            <ul class="w-fit">
              <li v-for="(item, index) in tocList"
                  :key="`${item}-${index}`"
                  @click="handleScrollSection(index)"
